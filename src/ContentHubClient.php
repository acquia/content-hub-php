...
class ContentHubClient extends Client {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $config = [], LoggerInterface $logger, Settings $settings, HmacAuthMiddleware $middleware, EventDispatcherInterface $dispatcher, $api_version = 'v2') { // phpcs:ignore
    $this->logger = $logger;
  ...
  public static function register(
    ...
    try {
      ...
      $settings = new Settings($values['name'], $values['uuid'], $api_key,
        $secret, $url);
      $config = [
        'base_url' => $settings->getUrl(),
      ];
      $client = new static($config, $logger, $settings,
      ...
      $settings = new Settings($settings->getName(), $settings->getUuid(),
        $settings->getApiKey(), $settings->getSecretKey(), $settings->getUrl(),
   ...
   *   Response object.
   */
  public function createEntities(CDFObject ...$objects) { // phpcs:ignore
    $json = [
      'resource' => "",
    ];
   ...
   *
   * The entity does not need to be passed to this method, but only the
   * resource URL.
   *
   ...
   *   Response object.
   */
  public function putEntities(CDFObject ...$objects) { // phpcs:ignore
    $json = [
      'resource' => "",
    ];
    foreach ($objects as $object) {
   ...
   *   Response object.
   */
  public function postEntities(CDFObject ...$objects) { // phpcs:ignore
    $json = [
      'resource' => "",
    ];
    foreach ($objects as $object) {
   ...
   * @throws \GuzzleHttp\Exception\RequestException
   */
  public function logs($query = '', $query_options = []) { // phpcs:ignore
   ...
   * @throws \Exception
   */
  public function listEntities($options = []) { // phpcs:ignore
    $variables = $options + ['limit' => 1000, 'start' => 0, 'filters' => []];
   ...
   * @throws \GuzzleHttp\Exception\RequestException
   */
  public function searchEntity($query) { // phpcs:ignore
    $options['body'] = json_encode((array) $query);
  ...
  public function getWebHook($url) {
    $webhooks = array_filter($this->getWebHooks(),
   ...
   * @throws \GuzzleHttp\Exception\RequestException
   */
  public function addEntitiesToInterestList($webhook_uuid, $uuids) { // phpcs:ignore
  ...
  public function regenerateSharedSecret() {
    return $this->getResponseJson($this->post('settings/secret',
      ['body' => json_encode([])]));
  }
   ...
   * @throws \Exception
   */
  public function putFilter($query, $name = '', $uuid = NULL, $metadata = []) { // phpcs:ignore
    $data = [
      'name' => $name,
      'data' => [
        'query' => $query,
  ...
  public function addFilterToWebhook($filter_id, $webhook_id) {
    ...
    return $this->getResponseJson($this->post("settings/webhooks/$webhook_id/filters",
      $options));
  }
  ...
  protected function getExceptionMessage(
    ...
    if ($exception instanceof ClientException || $exception instanceof BadResponseException) {
      ...
      switch ($method) {
        ...
        case 'deleteWebhook':
          ...
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('There was a problem trying to <b>unregister</b> Webhook URL = %s. Please try again. (Error Code = %d: @%s)',
              $webhook_url, $response->getStatusCode(),
              $response->getReasonPhrase()));

        case 'purge':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error purging entities from the Content Hub [Error Code = %d: %s]',
              $response->getStatusCode(), $response->getReasonPhrase()));

        case 'readEntity':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error reading entity with UUID="%s" from Content Hub (Error Code = %d: %s)',
              $args[0], $response->getStatusCode(),
              $response->getReasonPhrase()));

        case 'createEntity':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error trying to create an entity in Content Hub (Error Code = %d: %s)',
              $response->getStatusCode(), $response->getReasonPhrase()));

        case 'createEntities':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error trying to create entities in Content Hub (Error Code = %d: %s)',
              $response->getStatusCode(), $response->getReasonPhrase()));

        case 'updateEntity':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error trying to update an entity with UUID="%s" in Content Hub (Error Code = %d: %s)',
              $args[1], $response->getStatusCode(),
              $response->getReasonPhrase()));
        ...
        case 'deleteEntity':
          return $this->getErrorResponse($response->getStatusCode(),
            sprintf('Error trying to delete entity with UUID="%s" in Content Hub (Error Code = %d: %s)',
              $args[0], $response->getStatusCode(),
              $response->getReasonPhrase()));
    ...
    if ($exception instanceof RequestException) {
      switch ($method) {
        ...
        case 'createEntity':
          $message = sprintf('Error trying to create an entity in Content Hub (Error Message: %s)',
            $exception->getMessage());
          break;

        case 'createEntities':
          $message = sprintf('Error trying to create entities in Content Hub (Error Message = %s)',
            $exception->getMessage());
          break;

        case 'updateEntity':
          $message = sprintf('Error trying to update entity with UUID="%s" in Content Hub (Error Message = %s)',
            $args[1], $exception->getMessage());
          break;

        case 'updateEntities':
          $message = sprintf('Error trying to update some entities in Content Hub (Error Message = %s)',
            $exception->getMessage());
          break;

        case 'deleteEntity':
          $message = sprintf('Error trying to delete entity with UUID="%s" in Content Hub (Error Message = %s)',
            $args[0], $exception->getMessage());
          break;

        case 'searchEntity':
          $message = sprintf('Error trying to make a search query to Content Hub. Are your credentials inserted correctly? (Error Message = %s)',
            $exception->getMessage());
          break;

        default:
          $message = sprintf('Error trying to connect to the Content Hub. Are your credentials inserted correctly? (Error Message = %s)',
            $exception->getMessage());
          break;
      }

      return $this->getErrorResponse(500, $message);
    }

    $message = sprintf('Error trying to connect to the Content Hub (Error Message = %s)',
      $exception->getMessage());
  ...
  /**
   * Strip all leading and trailing slashes from components of the given array.
   *
